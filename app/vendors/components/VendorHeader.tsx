...
const VendorHeader = ({ onMenuClick, isSidebarCollapsed }: VendorHeaderProps) => {
  ...
  return (
    <nav className={`
      fixed top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 h-20 flex items-center transition-all duration-300
      left-0 md:${isSidebarCollapsed ? "left-16" : "left-24"}
      right-0 lg:pr-[350px]
    `}>
      <div className="w-full flex items-center gap-4">
        <button 
          onClick={onMenuClick}
          className="p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-50 rounded-xl md:hidden transition-colors"
        >
        ...
        <button 
          onClick={() => scroll('left')}
          className="p-1 hover:bg-slate-100 rounded-full transition-colors hidden lg:flex"
        >
        ...
        >
          {categories.map((cat) => (
            <Link
              key={cat.name}
              href={cat.href}
              className="whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-bold text-slate-600 hover:text-primary hover:bg-primary/5 transition-all"
            >
              {cat.name}
            </Link>
          ))}
